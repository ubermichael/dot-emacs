...
;; Created: 2005-08-03
;; Last-Updated: Sat Apr 29 16:11:09 2006 (7200 +0200)
(setq html-pagetoc:version "0.84") ;; Version:
      ...
      (let ((minmax-patt (format html-pagetoc-maxmin-cmnt "\\([[:alnum:]]+\\)" "\\([[:alnum:]]+\\)")))
        (save-excursion 
          (when (search-forward-regexp minmax-patt nil t)
            (setq old-min (string-to-int (match-string 1)))
            (setq old-max (string-to-int (match-string 2))))))
      (let ((start-toc (point)))
...
maximum level of h1-h6 to include.  They should be integers."
  ...
  (let* ((curr-buffer (current-buffer))
	...
	(let ((b (current-buffer))
	  ...
	  (princ "<strong>On this page:</strong>\n")
	  (while (re-search-forward
		  (concat "\\(?:<h\\([1-9]\\)\\([^>]*\\)>\\(.*?\\)</h[1-9]>"
			  "\\|"
			  "<!--\\(?:.\\|\n\\)-->\\)")
		  nil t)
	    ;; There seem to be a bug in the regexp in Emacs 21.3.1
	    ;; here, no match of <!-- Or rather there seem to be a
	    ;; serious error that makes Emacs crash. After restarting
	    ;; Emacs it works...
	    ;;(read-from-minibuffer (format "here=%s" (buffer-substring (- (point) 3) (point))))
	    ;;(unless (equal "<!--" (buffer-substring (- (point) 3) (point)))
