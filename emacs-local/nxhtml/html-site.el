...
;; Copyright (C) 2006 by Lennart Borgman

;; Author: Lennart Borgman <lennartDOTborgmanDOT073ATstudentDOTluDOTse>
;; Created: Wed Mar 01 17:25:52 2006
(defconst html-site:version "0.1");; Version:
;; Last-Updated: Wed May 17 01:49:12 2006 (7200 +0200)
;; Keywords:
;; Compatibility:
;;
...
(defun html-site-ensure-site-defined()
  ...
  (unless (file-directory-p (html-site-current-site-dir))
...
(defun html-site-ensure-buffer-in-site()
  ...
  (html-site-ensure-file-in-site buffer-file-name))

(defun html-site-ensure-file-in-site(file-name)
  (html-site-ensure-site-defined)
  (unless (html-site-current-contains file-name)
...
(defun html-site-local-to-web(local-file with-toc)
  (html-site-ensure-file-in-site local-file)
  (html-site-path-in-mirror (html-site-current-site-dir)
                            local-file
                            (html-site-current-web-full with-toc)))

(defun html-site-remote-root(with-toc)
  (concat "/ftp:"
          (html-site-current-ftp-user)
          "@" (html-site-current-ftp-host)
          ":"
...
(defun html-site-local-to-remote(local-file with-toc)
  (html-site-ensure-file-in-site local-file)
  (html-site-path-in-mirror (html-site-current-site-dir)
                            local-file
                            (html-site-remote-root with-toc)))
...
(defun html-site-create-pages-file()
...
Use the entry with this name in `html-site-list'."
  :set (lambda(symbol value)
         (or (when (= 0 (length value))
               (lwarn '(html-site-current) :warning "No current site"))
             (let ((site-names))
               (dolist (m html-site-list)
