...
;; Last-Updated: Wed May 17 01:50:29 2006 (7200 +0200)
;; Keywords:
;; Compatibility:
;;
;; Features that might be required by this library:
;;
;;   `cl', `fupd', `html-site', `html-toc'.
;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;
;;; Commentary:
...
"
  ...
  (let ((pag-file (html-site-current-page-list))
    ...
    (unless (file-exists-p pag-file)
      (error "Can't find page file for site %s.\nHave you done M-x html-site-create-pages-file?"
             html-site-current))
    (unless (< 0 (length tpl-file))
      (error "Template file not defined for site %s.\nPlease use customize to add this in `html-site-list'."
             html-site-current))
    (let (
          (buffer (noshell-procbuf-setup "subprocess for merging pages and TOC"))
          (opt (list
